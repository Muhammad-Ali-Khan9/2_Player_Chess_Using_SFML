...
bool Pawn::canCapture(Piece* capture_piece)
{
	...
	if (*capture_piece->m_player != *m_player) {
		if (abs(capture_piece->PieceSquare->x - PieceSquare->x) == 1 && abs(capture_piece->PieceSquare->x - PieceSquare->y) == 1) {
			firstStride = false;
			return true;
		}
	}
...
bool Pawn::isValidPath(int finalX,int finalY)
{
	...
	if (abs(finalX - PieceSquare->x) != 1 || abs(finalX - PieceSquare->y) != 0)
		return false;

	int x_dir = (m_player->colour == Black) ? 1 : -1;
