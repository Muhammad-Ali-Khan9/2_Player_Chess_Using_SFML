...
void GUI::update() {
    ...
    for (int i = 0; i < 8; i++) {
        for (int j = 0; j < 8; j++) {
            if (chessGame.board.board[i][j]->hasPiece) {
                Piece* piece = chessGame.board.getPiece(chessGame.board.board[i][j]);
    ...
    while (window.isOpen())
    {
        while (window.pollEvent(event))
        {
            ...
            if (!selected && event.type == sf::Event::MouseButtonPressed) {
                ...
                if (chessGame.board.board[selected_posX][selected_posY]->hasPiece) {
                    selected_piece = chessGame.board.getPiece(chessGame.board.board[selected_posX][selected_posY]);
            ...
            if (selected && event.type == sf::Event::MouseButtonPressed) {
                ...
                /* if there is a piece*/
                if (chessGame.board.board[mouse_pos.y / 60][mouse_pos.x / 60]->hasPiece) {
                    //get the destination's piece
                    Piece* clicked_piece = chessGame.board.getPiece(chessGame.board.board[mouse_pos.y / 60][mouse_pos.x / 60]);
