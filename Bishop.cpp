...
{
	if (isValidPath(capture_piece->PieceSquare->x, capture_piece->PieceSquare->y) && *capture_piece->m_player != *m_player)
		return true;
...
{
	return (abs(finalX - PieceSquare->x) == abs(finalX - PieceSquare->y));
}
...
void Bishop::drawPath(int finalX,int finalY)
{
	...
	path[1].resize(abs(finalY - PieceSquare->y));
